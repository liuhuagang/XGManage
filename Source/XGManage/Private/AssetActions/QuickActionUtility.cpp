...
void UQuickActionUtility::DuplicateAssets(int32 NumOfDuplicates)
{
	...
	for (const FAssetData& SelectedAssetData : SelectedAssetsData)
	{
		for (int32 i = 0; i < NumOfDuplicates; i++)
		{
			...
			const FString NewDuplicatedAssetName
				= SelectedAssetData.AssetName.ToString() + TEXT("_") + FString::FromInt(i + 1);
...
void UQuickActionUtility::RemoveUnusedAssets()
{
	...
	{
		DebugHeader::ShowNotifyInfo(TEXT("Successfully deleted" + FString::FromInt(NumOfAssetsDeleted)
			+ TEXT(" unused assets")));
	}
